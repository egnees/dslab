...
/// Specifies the behavior of timer set in the presence of existing active timer with this name.
...
pub(crate) struct ProcessEntry {
    ...
    /// Represents actions, which were generated by process,
    /// but still not handled by node.
    pub(crate) pending_actions: Rc<RefCell<Vec<ProcessEvent>>>,
}
...
impl ProcessEntry {
    pub fn new(proc_impl: Box<dyn Process>) -> Self {
        Self {
            ...
            pending_actions: rc!(refcell!(Vec::default())),
        }
    }

    pub fn extract_pending_actions(&mut self) -> Vec<ProcessEvent> {
...
impl Node {
    ...
    fn on_sleep_finished(&mut self, proc: String) {
        ...
        // Do not need to handle actions here.
    }
...
impl EventHandler for Node {
    fn on(&mut self, event: Event) {
        cast!(match event.data {
            ...
            SleepStarted { proc, duration } => {
                self.on_sleep_started(proc, duration);
            }
            SleepFinished { proc } => {
                self.on_sleep_finished(proc);
            }
            ActivityFinished { proc } => {
                self.on_async_finished(proc);
            }
            MessageAck { id, delivered } => {
                println!("node got message ack: {}, {}", id, delivered);
            }
        })
    }
}
