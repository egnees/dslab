...
impl Process for ReliableSender {
    ...
    fn on_local_message(&mut self, msg: Message, ctx: Context) -> Result<(), String> {
        ...
        ctx.clone().spawn(async move {
            let pair = pair;
            ctx.send_with_ack(msg, &pair, 20.0).await.unwrap();
        });
...
fn reliable_works() {
    ...
    assert!(!system.read_local_messages("s1").unwrap().is_empty());
    assert!(!system.read_local_messages("s2").unwrap().is_empty());
    ...
    assert!(system.read_local_messages("s2").is_none());
    assert!(system.read_local_messages("s2").is_none());
}
